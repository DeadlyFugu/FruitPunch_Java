...
public static String[] parse(String in) {
	...
	for(char c : in.toCharArray()) {
		if (cm) {
			...
			case '\\':
				if (str || csyn) {break;}
				else System.err.println("SYNTAX: WHY IS \\ HERE?"); break;
			case '[':
				if (str || csyn) {token.append(c); break;}
				csyn=true;
				token.append(c);
			...
			case '\"':
				if (str) {
					...
					if (token.length()!=0) {
						tokens.add(token.toString());
						token.setLength(0);
						System.err.println("WARN: Mid-line quote");
					}
					str=true;
					token.append(c);
				} else {
			...
			System.err.println("SYNTAX: WHY IS \\ HERE?");
		}
		p=c;
	}
	if (str || csyn) {
		System.err.println("WARN: unclosed string or macro");
	}
